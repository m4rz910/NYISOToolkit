...
class NYISOData:
    ...
    def construct_database(self):
        ...
        if not files:
            ...
            frames = [
                pd.read_csv(file, index_col=0) for file in files
            ...
            if ("Time Zone" in df.columns) or (self.dataset_details.col is None):
                ...
                df = df.sort_index(axis="index").tz_convert(
                    "UTC"
                )  # Convert to UTC so that pivot can work without throwing error for duplicate indices
            ...
            df = df.tz_convert("US/Eastern").loc[
                start:end
            ]  # Convert back to US/Eastern to select time
...
def construct_databases(
    years, datasets, redownload=False, reconstruct=False, create_csv=False
):
