...
class NYISOStat:
    ...
    def table_average_day_load(self):
        ...
        df2 = self.table_hourly_dataset(dataset='fuel_mix_5m',
                                        year=self.year)[['Wind','Other Renewables']].sum(axis='columns')
    ...
    def table_annual_energy(self):
        ...
        imports.drop(('External Flows', 'SCH - HQ IMPORT EXPORT', 'Flow (MW)'),
                     axis='columns', inplace=True) #HQ Net is a subset of another external flow
    ...
    def table_instate_flow(self):
        ...
        df: Dataframe
            Energy table [GWh]
        """
        df = NYISOData(dataset='interface_flows_5m', year=self.year).df.tz_convert('US/Eastern')
