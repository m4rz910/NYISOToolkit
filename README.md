...
## NYISOVis
The visualizations are focused on communicating New York's status toward achieving the decarbonization goals outlined by the Climate Leadership and Community Protection Act (CLCPA).
"No later than [June 13, 2021], the commission shall establish a program to require that:
* (A) A minimum of [70%] of the state wide electric generation secured by jurisdictional load serving entities to meet the electrical energy requirements of all end-use customers in New York State in [2030] shall  be generated by  renewable  energy  systems [(70x2030)];
...
There are three kinds of visualizations currently supported, check them all out in the visualizations folder.
### Usage Example
```python
